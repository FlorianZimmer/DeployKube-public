...
KEYCLOAK_URL="${KEYCLOAK_URL:-http://mkc-keycloak:8080}"
ADMIN_USER="${MKC_ADMIN_USERNAME}"
ADMIN_PASSWORD="${MKC_ADMIN_PASSWORD}"
REALM="mkc"
...
ensure_service_client() {
  ...
  for role in manage-users view-users query-users manage-clients view-clients query-groups manage-identity-providers manage-realm view-events; do
    kcadm.sh add-roles -r "${REALM}" --uid "${sa_user_id}" --cclientid realm-management --rolename "${role}" >/dev/null 2>&1 || true
  done
}

ensure_oidc_idp() {
  if kcadm.sh get "identity-provider/instances/ukc" -r "${REALM}" >/dev/null 2>&1; then
    kcadm.sh delete "identity-provider/instances/ukc" -r "${REALM}" >/dev/null
  fi
  kcadm.sh create identity-provider/instances -r "${REALM}" \
    ...
    -s config.issuer=http://ukc-keycloak:8080/realms/ukc \
    -s config.discoveryUrl=http://ukc-keycloak:8080/realms/ukc/.well-known/openid-configuration \
    -s config.authorizationUrl=http://ukc-keycloak:8080/realms/ukc/protocol/openid-connect/auth \
    -s config.tokenUrl=http://ukc-keycloak:8080/realms/ukc/protocol/openid-connect/token \
    -s config.logoutUrl=http://ukc-keycloak:8080/realms/ukc/protocol/openid-connect/logout \
    -s config.userInfoUrl=http://ukc-keycloak:8080/realms/ukc/protocol/openid-connect/userinfo \
    -s config.jwksUrl=http://ukc-keycloak:8080/realms/ukc/protocol/openid-connect/certs \
    -s config.clientId=mkc-broker \
    -s config.clientSecret=MKC-Broker-123! \
...
login
ensure_realm
ensure_client "btp-broker" "BTP-Broker-123!" "http://btp-keycloak:8080/realms/btp/broker/mkc/endpoint"
ensure_service_client "mkc-scim-facade" "${MKC_CLIENT_SECRET}"
ensure_oidc_idp
ensure_existing_account_only_first_broker_login
echo "mkc configured"
