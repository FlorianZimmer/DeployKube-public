...
for overlay in "${overlay_dirs[@]}"; do
  ...
  if ! bash "${script_dir}/validate-prohibited-kinds.sh" "${out}"; then
    failures=$((failures + 1))
  fi

  if ! bash "${script_dir}/validate-namespace-boundary.sh" --org-id "${org_id}" --project-id "${project_id}" "${out}"; then
    failures=$((failures + 1))
  fi

  if ! bash "${script_dir}/validate-policy-aware-lint.sh" --org-id "${org_id}" "${out}"; then
    failures=$((failures + 1))
  fi
done
...
if [[ "${skip_secret_scan}" != "true" ]]; then
  ...
  if ! bash "${script_dir}/validate-no-secrets-in-git.sh" --source "${repo_root}"; then
    failures=$((failures + 1))
  fi
fi
